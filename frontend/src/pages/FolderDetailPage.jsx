...
const EditDocumentModal = ({ isOpen, onClose, onConfirm, doc, allTags }) => {
    ...
    useEffect(() => {
        if (doc) {
            setName(doc.name);
            // Safely map tags: filter out any that might not be found (e.g., if deleted)
...
const FolderDetailPage = () => {
    ...
    const fetchFolderData = async () => {
        try {
            ...
            const docsData = Array.isArray(docsRes.data.results) ? docsRes.data.results : [];
            const tagsData = Array.isArray(tagsRes.data) ? tagsRes.data.map(t => ({ value: t.id, label: t.name })) : [];
    ...
    const handleDeleteDocument = async () => {
        ...
        fetchFolderData(); // Refresh list after deleting
    };
        ...
        <>
            ...
            <div className="container mx-auto px-4 py-8">
                    ...
                    <form onSubmit={handleUpload} className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            />
                             <input
                                type="file"
                                key={uploadFile ? 'file-selected' : 'no-file'} // Reset input on clear
                                onChange={(e) => {
                ...
                <div className="space-y-4">
                    {documents.length > 0 ? (
                        documents.map(doc => (
                            <div key={doc.id} className="bg-gray-800 rounded-lg p-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                                ...
                                <div className="flex items-center gap-4 w-full md:w-auto">
                                   <div className="flex-grow text-sm text-gray-400">
                                    ...
                                    <button onClick={() => setModalState({ type: 'edit_document', data: doc })} className="icon-button"><FaEdit /></button>
                                    <button onClick={() => setModalState({ type: 'delete_document', data: doc })} className="icon-button text-red-500"><FaTrash /></button>
